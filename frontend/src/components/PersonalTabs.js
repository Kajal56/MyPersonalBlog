...
export default function PersonalTabs() {
  ...
  <div className="bg-white dark:bg-gray-900 shadow-lg border border-blue-100 dark:border-gray-800 px-2 py-2 flex gap-2 md:gap-4" style={{ minWidth: 'fit-content', maxWidth: '100%', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
        {tabs.map(tab => (
          <Link
            key={tab.href}
            href={tab.href}
            className={`px-4 py-2 text-sm font-medium transition-colors duration-200
              ${pathname === tab.href
                ...
                : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-blue-900'}
            `}
          >
            {tab.label}
          </Link>
